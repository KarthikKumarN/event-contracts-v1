...
describe("BukRoyalties Updations", function () {
  ...
  describe("Set Buk Protocol in BukRoyalties", function () {
    it("Should set buk protocol by admin", async function () {
      //Set buk protocol
      expect(
        await royaltiesContract.connect(adminWallet)
        .setBukProtocolContract(account1)
        ).not.be.reverted;
      const addr = await royaltiesContract.connect(adminWallet).bukProtocolContract()
    ...
    it("Should not set buk protocol if not admin", async function () {
      //Set buk protocol
      await expect(royaltiesContract.connect(account2).setBukProtocolContract(account1)).to.be.reverted;
    });
  });
  ...
  describe("Set Buk Royalty in BukRoyalties", function () {
    it("Should set Buk Royalty by admin", async function () {
      //Set Buk Royalty
      expect(await royaltiesContract.connect(adminWallet).setBukRoyaltyInfo(bukTreasuryContract, 200)).not.be.reverted;
    ...
    it("Should set Buk Royalty and emit events", async function () {
      //Set Buk Royalty
      expect(await royaltiesContract.connect(adminWallet).setBukRoyaltyInfo(bukTreasuryContract, 200))
        .to.emit(royaltiesContract, "SetStableToken")
        .withArgs(await account1.getAddress());
    });
    it("Should not set Buk Royalty if not admin", async function () {
      //Set Buk Royalty
      await expect(royaltiesContract.connect(account1).setBukRoyaltyInfo(bukTreasuryContract, 200)).to.be.reverted;
    });
    it("Should not set Buk Royalty if royalty fee is more than 10000", async function () {
      //Set Buk Royalty
      await expect(royaltiesContract.connect(adminWallet).setBukRoyaltyInfo(bukTreasuryContract, 20000)).to.be.revertedWith("Royalty fraction is more than 10000");
    });
  });
  ...
  describe("Set Hotel Royalty in BukRoyalties", function () {
    it("Should set Hotel Royalty by admin", async function () {
      //Set Hotel Royalty
      expect(await royaltiesContract.connect(adminWallet).setHotelRoyaltyInfo(bukTreasuryContract, 200)).not.be.reverted;
    ...
    it("Should set Hotel Royalty and emit events", async function () {
      //Set Hotel Royalty
      expect(await royaltiesContract.connect(adminWallet).setHotelRoyaltyInfo(bukTreasuryContract, 200))
        .to.emit(royaltiesContract, "SetStableToken")
        .withArgs(await account1.getAddress());
    });
    it("Should not set Hotel Royalty if not admin", async function () {
      //Set Hotel Royalty
      await expect(royaltiesContract.connect(account1).setHotelRoyaltyInfo(bukTreasuryContract, 200)).to.be.reverted;
    });
    it("Should not set Hotel Royalty if royalty fee is more than 10000", async function () {
      //Set Hotel Royalty
      await expect(royaltiesContract.connect(adminWallet).setHotelRoyaltyInfo(bukTreasuryContract, 20000)).to.be.revertedWith("Royalty fraction is more than 10000");
    });
  });
  ...
  describe("Set First Owner Royalty in BukRoyalties", function () {
    it("Should set First Owner Royalty by admin", async function () {
      ...
      expect(await royaltiesContract.connect(adminWallet).setFirstOwnerRoyaltyInfo(200)).not.be.reverted;
    ...
    it("Should set First Owner Royalty and emit events", async function () {
      //Set First Owner Royalty
      expect(await royaltiesContract.connect(adminWallet).setFirstOwnerRoyaltyInfo(200))
        .to.emit(royaltiesContract, "SetStableToken")
        .withArgs(await account1.getAddress());
    });
    it("Should not set First Owner Royalty if not admin", async function () {
      //Set First Owner Royalty
      await expect(royaltiesContract.connect(account1).setFirstOwnerRoyaltyInfo(200)).to.be.reverted;
    });
    it("Should not set First Owner Royalty if royalty fee is more than 10000", async function () {
      //Set First Owner Royalty
      await expect(royaltiesContract.connect(adminWallet).setFirstOwnerRoyaltyInfo(20000)).to.be.revertedWith("Royalty fraction is more than 10000");
    });
  });
  ...
  describe("Set other Royalties in BukRoyalties", function () {
    it("Should set other Royalty by admin", async function () {
      ...
      expect(await royaltiesContract.connect(adminWallet).setOtherRoyaltyInfo(recipients, royaltyFractions)).not.be.reverted;
    ...
    it("Should set other Royalty and emit events", async function () {
      ...
      const recipients = [await account1.getAddress(), await account2.getAddress()]
      const royaltyFractions = [2000, 3000]
      expect(await royaltiesContract.connect(adminWallet).setOtherRoyaltyInfo(recipients, royaltyFractions))
    ...
    it("Should set and replace the existing other royalties by admin", async function () {
      ...
      expect(await royaltiesContract.connect(adminWallet).setOtherRoyaltyInfo(recipients, newRoyaltyFractions)).not.be.reverted;
    ...
    it("Should not set other Royalty if not admin", async function () {
      ...
      const recipients = [await account1.getAddress(), await account2.getAddress()]
      const royaltyFractions = [2000, 3000]
      await expect(royaltiesContract.connect(account1)
    ...
    it("Should not set other Royalty if array size mismatch is there", async function () {
      ...
      await expect(royaltiesContract.connect(adminWallet)
        .setOtherRoyaltyInfo(recipients, royaltyFractions))
