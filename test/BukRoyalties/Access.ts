...
describe("BukProtocol Access Control", function () {
  ...
  describe("Manage Default Admin Role", function () {
    it("Should set new default admin", async function () {
      ...
      expect(
        await royaltiesContract.connect(adminWallet).grantRole(
          DEFAULT_ADMIN_ROLE,
          account1
        ),
      ).not.be.reverted;
      ...
      expect(
        await royaltiesContract.connect(adminWallet).hasRole(
          DEFAULT_ADMIN_ROLE,
          account1
        ),
      ).to.equal(true)
    ...
    it("Should set new default admin and revoke old one", async function () {
      ...
      expect(
        await royaltiesContract.connect(adminWallet).revokeRole(
          DEFAULT_ADMIN_ROLE,
          account1
        ),
      ).not.be.reverted;
    ...
    it("Should set new default admin and check function with new one", async function () {
      ...
      await expect(
        royaltiesContract.connect(adminWallet).grantRole(
          DEFAULT_ADMIN_ROLE,
      ...
      // Check if the new admin has DEFAULT_ADMIN_ROLE
      expect(
        await royaltiesContract.connect(account1).hasRole(
          DEFAULT_ADMIN_ROLE,
          await account1.getAddress()
        )
      ).to.equal(true);
      ...
      await expect(
        royaltiesContract.connect(account1).grantRole(
          DEFAULT_ADMIN_ROLE,
          await account1.getAddress()
        )
      ).to.not.be.reverted;
